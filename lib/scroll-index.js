...
module.exports = (elastic, opts, cb) => {
  ...
  const onHit = opts.onHit || ((hit, cb) => cb(null, hit));
  ...
  const searchOpts = {
    index: 'ciim',
    size: opts.pageSize,
    scroll: '1m', // Set to 30 seconds because we are calling right back
    body: {
      query: {
        bool: {
          must: {
  ...
  elastic.search(searchOpts, function scroll (err, res) {
    ...
    Async.eachSeries(res.body.hits.hits, iterate, (err) => {
      if (err) return cb(err);
      if (res.body.hits.total.value === numProcessed) {
      ...
      elastic.scroll({ scrollId: res.body._scroll_id, scroll: '1m' }, scroll);
    });
  });
};
